...
class SendMessageTypeRouter: SendMessageTypeWireframe {
    weak var viewController: UIViewController?
    
    static func assembleModule(_ sendTransaction: SendTransaction) -> UIViewController {
        let view = R.storyboard.sendMessageTypeStoryboard.sendMessageTypeView()!
        let presenter = SendMessageTypePresenter()
        let interactor = SendMessageTypeInteractor()
        let router = SendMessageTypeRouter()
